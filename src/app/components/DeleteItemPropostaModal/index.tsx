...
import { deleteItemProposta } from '@/app/services/delete/DeleteItemPropostaService';
...
export default function DeleteItemPropostaModal({ title, id }: CreateParecerComercialModalProps) {
  const [isOpen, setIsOpen] = useState(false);
 
  const queryClient = useQueryClient();

  const mutation = useMutation({
    mutationFn: deleteItemProposta,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['propostas'] });
      setIsOpen(false);
      toast.success('Item da proposta excluido.');
    },
    onError: (error) => {
      toast.error('Erro ao excluir item da proposta');
    },
  });
